...
Cell AllocateCells(int n, int maxneighbour) {
	...
	if((cells.t_debut_division=(int *)calloc((size_t)n,sizeof(int)))==NULL) {
		fprintf(stderr,"error in memory allocation\n");
		exit(EXIT_FAILURE);
	}	
...
int* AllocateLineInterface(int n){
	int* line_interface;
	if((line_interface=(int *)calloc((size_t)n,sizeof(int)))==NULL){
		fprintf(stderr,"error in memory allocation for line_interface\n");	
		exit(EXIT_FAILURE);
	}
